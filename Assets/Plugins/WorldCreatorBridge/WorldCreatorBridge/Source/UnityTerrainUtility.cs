...
namespace BtB.WC.Bridge
{
    ...
    {
        //Переменные
        public static string FolderPath => BtB.WC.Bridge.PathProvider.GetDirectoryPath();
        //работаем тут
        ...
        private static Terrain[,] ConstructTerrainPart(ParamsBridge pb, ParamsImport pi, Transform container)
        {
            ...
            for (int tileX = 0; tileX < stepsX; tileX++)
            {
                ...
                for(int tileY = 0; tileY < stepsY; tileY++)
                {
                    ...
                    {
                        //иногда пути слетают если ставить бридж не по дефолту, а в свою папку
                        
                        Material mat = AssetDatabase.LoadAssetAtPath<Material>(FolderPath + "/Content/Materials/WC_Default_Terrain_" + pb.materialType + ".mat");
                        if (mat == null) Debug.LogError("Could not find material в указанной папке");
                        Texture2D tex;
                        if (pb.isImportLayers)
                            tex = AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/" + pb.terrainsFolderName + "/" + pb.assetName + "/colormap" + pi.nameEnding + ".png");
                        else
                            tex = AssetDatabase.LoadAssetAtPath<Texture2D>(FolderPath + "/Content/Textures/albedo_default.png");
            ...
            foreach (XmlElement splatmap in texturing)
            {
                foreach (XmlElement textureInfo in splatmap.ChildNodes)
                {
                    ...
                    if (pi.HasTexture("AlbedoFile"))
                    {
                        ...
                        layer.diffuseTexture = AssetDatabase.LoadAssetAtPath<Texture2D>(FolderPath + "/Content/Textures/albedo_default.png");
                    ...
                    if (pi.HasTexture("NormalFile"))
                    {
                        ...
                        layer.normalMapTexture = AssetDatabase.LoadAssetAtPath<Texture2D>(FolderPath + "/Content/Textures/normal_default.png");
                    ...
                    if (pb.materialType != MaterialType.Standard && (pi.HasTexture("AoFile") || pi.HasTexture("RoughnessFile")))
                    {
                        ...
                        ComputeShader maskMapConversionCs = AssetDatabase.LoadAssetAtPath<ComputeShader>(FolderPath + "/Content/Shaders/Compute/MaskMap.compute");
                    ...
                    {
                        Texture2D baseMask = AssetDatabase.LoadAssetAtPath<Texture2D>(FolderPath + "/Content/Textures/mask_default.png");
        ...
        private static void FillMaterial_Single(ParamsBridge pb, ParamsImport pi, ref Terrain[,] parts)
        {
            ...
            Texture2D baseNormal = AssetDatabase.LoadAssetAtPath<Texture2D>(FolderPath + "/Content/Textures/normal_default.png");
            Texture2D baseMask = AssetDatabase.LoadAssetAtPath<Texture2D>(FolderPath + "/Content/Textures/mask_default.png");
